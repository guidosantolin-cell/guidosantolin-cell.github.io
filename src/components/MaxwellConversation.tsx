...
import type { MaxwellConversationConfig } from '../data/maxwellConversations'
...
export function MaxwellConversation({ config }: { config: MaxwellConversationConfig }) {
  const { script, widget, finalReply, highlightPhrases } = config
      ...
      >
        ...
        {visible.some((t) => t.from === 'widget') && (
          <div className="mt-1 w-full rounded-2xl border border-neutral-200 bg-white p-4 shadow-md">
            ...
            <p className="mt-3 text-base font-semibold text-neutral-900">{widget.title}</p>
        ...
        {showFinal && (
          <div className="max-w-[80%] animate-[fadeIn_0.3s_ease] self-start whitespace-pre-line rounded-2xl rounded-bl-sm border border-neutral-200 bg-white px-3.5 py-2.5 text-sm text-neutral-800">
            {renderRich(finalReply, highlightPhrases)}
          </div>
        )}
